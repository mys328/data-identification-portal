...
<div modal-dialog
     ...
     >
    <div>
        <input ng-model="treeDialogView.filterField" ng-change="delayReloadTree()" type="text" class="inputText_lg"/>
        <button ng-click="delayReloadTree()" type="button" class="btn btn-default"><i class="fa fa-search"></i> 查 找</button>
    </div>
    ...
    <div class="tab-content" style="border:1px solid #e1e1e1; border-top:0;" id="{{dialogId}}-treeDialogByUser">
        <div role="tabpanel" class="tab-pane" ng-class="{'active': !treeDialogView.all}">
            ...
            <ol style="list-style: none;height: 222px;overflow: auto; cursor: pointer;padding:10px 0 0;margin-bottom:0;">
                <li style="float: left; margin: 5px;"
                    ...
                    ng-dblclick="dbSelected(userfulItem)">{{userfulItem.userName}}&nbsp;&nbsp;|</li>
            </ol>
        </div>
    	<div role="tabpanel" class="tab-pane" ng-class="{'active': treeDialogView.all}">
    		<div class="leftMenu" style="width:70%;">
		    	...
		    	<div class="tree-dialog" role="{{modalTitle}}" onselectstart="return false;" ng-if="!treeDialogView.filterField">
			        <ol style="height: 222px; overflow: auto; font-size: 16px; margin-left: 0px;padding:10px 0 0;margin-bottom:0;">
			            <li ng-repeat="node in nodes" ng-include="contextServer + '/ct/js/tpl/tree.user.node.jsp'"></li>
			        </ol>
			        <div mask visible="treeDialogView.showMask" ref-id=".tree-dialog[role='{{modalTitle}}']"></div>
		    	</div>-->
		    	<!-- 查询树 ng-if="treeDialogView.filterField"-->
		    	<div class="ht_info tree-dialog" >
				    <div style="height: 222px; overflow: auto; font-size: 16px; margin-left: 0px;padding:10px 0 0;margin-bottom:0;" 
				    	ng-include="contextServer + '/ct/js/tpl/tree.user.node.all.jsp'"></div>
				</div>
			</div>
			<!-- 人员展示 -->
			<div class="rightmenu_xg" style="width:30%;">
				<ol style="height: 222px; overflow: auto; font-size: 16px; margin-left: 0px;padding:10px 0 0;margin-bottom:0;">
		            <li ng-repeat="item in usersList" ng-click="selected(item)" ng-dblclick="dbSelected(item)"
			            style="cursor: pointer"
	                	ng-style="{'background-color' : (item == treeDialogView.selectedUser) ? 'darkgray': 'white', 
				         'padding' : (item == treeDialogView.selectedUser) ? '5px': '0',
				         'color': (item == treeDialogView.selectedUser) ? 'white' : 'black'}">
		            	{{item.userName}}
		            </li>
		            <li ng-if="!(usersList && usersList.length > 0)">
		            	无人员记录
		            </li>
		        </ol>
			</div>
    ...
    <div mask visible="treeDialogView.showMask" ref-id="<%= '#' %>{{dialogId}}-treeDialogByUser"></div>
</div>
